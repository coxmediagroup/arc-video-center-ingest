...
functions.doesObjectExist = async (bucket, key) => {
  ...
  try {
    ...
    console.log(`[/lib/s3.js::doesObjectExist()] ${err}`);
    return false;
  }
};
...
functions.getObjectContentsAsJsonString = async (bucket, key) => {
  ...
  try {
    ...
    console.log(`[/lib/s3.js::getObjectContentsAsJsonString()] [ERROR::ARC_VIDEO_CENTER_INGEST] Could not get contents of [${bucket}/${key}]. Error was:\n${err}`);
    return null;
  }
};
...
functions.addTags = async (Bucket, Key, TagSet) => {
  try {
    ...
    console.log(`[/lib/s3.js::addTags()] [ERROR::ARC_VIDEO_CENTER_INGEST] ${err}`);
  }
};
