...
functions.importFile = async (bucket, key, executionContext) => {
  ...
  try {
    ...
    console.error(err);
    console.log(`[/lib/importer.js::importFile()] [ERROR::ARC_VIDEO_CENTER_INGEST] There was an error processing key [${key}].\nError was: ${err}`);
...
functions.postAnsToVideoCenter = async (params) => {
  const url = `https://${params.domain}/goldfish/video/v2/import/ans?encode=${params.encode}`;
  ...
  const options = {
    ...
    body: JSON.stringify(params.ans),
  };

  console.log(`Posting ANS to [${url}]`);
