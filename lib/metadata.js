...
functions.getDefaultMetadata = async (bucket, key, context) => {
  console.log('getDefaultMetadata');
  ...
  console.log(defaultMetadataKeys);
...
functions.getMetadataForKey = async (bucket, key, context) => {
  ...
  try {
    ...
    console.log(`[/lib/metadata.js::getMetadataForKey()] [ERROR::ARC_VIDEO_CENTER_INGEST] Couldn't load file for [${key}]\nError was: ${err}`);
  }
